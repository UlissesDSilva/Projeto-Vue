module.exports = app => {
  ...
  const save = async (req, res) => {
    ...
    try {
      ...
      const categoryFormDB = await app.db('categories').where({name: category.name}).first()

      if (!category.id) {
        notExistsOrError(categoryFormDB, 'Categoria já cadastrada')
      }
    } catch (msg) {
  ...
  const remove = async (req, res) => {
    try {
      ...
      notExistsOrError(subCategory, 'Categoria possue subcategorias.')
  ...
  return { save, getAll, getById, remove, getTree }
}